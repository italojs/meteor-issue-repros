...
function extractResourcePath(source, sourceMapPath, rootPath) {
    if (source.startsWith('webpack://')) {
        ...
        if (match?.[1]) {
            ...
            return external_node_path_default().join(rootPath, matched);
        }
    }
    if (external_node_path_default().isAbsolute(source)) return source;
    return external_node_path_default().join(external_node_path_default().dirname(sourceMapPath), source);
}
async function tryGenerateErrorFromSourceMap({ err, code, rootPath, outputFilepath, relativeOutputPath }) {
    ...
    try {
        ...
        const relativeSourcePath = external_node_path_default().relative(rootPath, absoluteSourcePath);
...
exports.CheckSyntax = __webpack_exports__.CheckSyntax;
exports.CheckSyntaxRspackPlugin = __webpack_exports__.CheckSyntaxRspackPlugin;
exports.PLUGIN_CHECK_SYNTAX_NAME = __webpack_exports__.PLUGIN_CHECK_SYNTAX_NAME;
exports.pluginCheckSyntax = __webpack_exports__.pluginCheckSyntax;
for(var __webpack_i__ in __webpack_exports__)if (-1 === [
    "CheckSyntax",
    "CheckSyntaxRspackPlugin",
